...
<aside class="notes">
* you are the primary beneficiary of these approaches, but will at the same time adhere to principles of open science
...
<aside class="notes">
* achieves the goal of separating content from presentation, but is not very readable for a human reader (and thus editor)
</aside>
...
<aside class="notes">
* Tables (but now it gets complicated); show but then lead to RMarkdown; use mtcars or similar
* often table content is the result of data analysis, it could thus be dynamic and we may want to create it programmatically; this is where RMarkdown makes an entry
</aside>
...
Above is the default *YAML header* when generating an `R Markdown` file in R Studio.
...
We will only scratch the surface of this and focus on basic recipes and elements, namely:
